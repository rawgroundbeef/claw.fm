...
- Calculate artist amount (95%): `(total * 95n) / 100n`
- Calculate platform amount (5%): `total - artistAmount` (ensures no rounding loss)
...
`cd web && npx tsc --noEmit` passes. Hooks export correct signatures. ConfettiCelebration renders without errors.
  ...
  <done>
useUSDCTransfer hook handles 95/5 USDC split with toast notifications and error recovery. useDownloadPurchase hook handles buy flow with download URL retrieval. ConfettiCelebration component triggers canvas confetti animation on prop change. All type-check cleanly.
  </done>
</task>
...
Props: `{ artistWallet: string; trackId: number; disabled?: boolean }`
...
- Render a row of 3 buttons for TIP_AMOUNTS ($0.25, $1, $5):
  ...
  - On click: call sendTransfer(artistWallet, amount). On success, call POST /api/tip with { trackId, amount, txHash } to update rotation weight
...
- After successful tip POST /api/tip, fire a callback: `onTipSuccess?: () => void` prop (for confetti trigger)
...
`cd web && pnpm run build` succeeds. Dev server shows tip ($0.25, $1, $5) and buy ($2) buttons below track info when a track is playing. Buttons appear disabled when wallet is not connected.
  ...
  <done>
TipButtons renders 3 preset amounts, fires USDC transfers with 95/5 split, calls /api/tip on success, triggers confetti callback. BuyButton renders $2 buy option, shows download link on success. Both components disable when wallet not connected or insufficient balance. Payment area wired into App.tsx above player bar, visible only during active playback.
  </done>
</task>
...
5. Once connected, verify address and USDC balance are shown in header
6. When a track is playing, verify tip buttons ($0.25, $1, $5) and buy button ($2) appear below track info
7. If wallet has no USDC, verify buttons are disabled/grayed out
8. If on testnet/with test USDC: tap a tip button, verify toast shows "Sending tip...", then "Tip sent!" with confetti
9. Verify buy button shows "Download Ready" link after successful purchase
10. Check browser console for any errors
11. Verify existing player functionality still works (play/pause, volume, visualizer, crossfade)

Note: Full payment testing requires USDC on Base. UI/UX verification (layout, button states, disabled states, toast rendering) can be done without funds.
  </how-to-verify>
...
6. POST /api/tip called after successful tip transfer
7. POST /api/downloads/:trackId called after successful buy transfer
8. Existing player functionality unaffected
</verification>
...
- PAY-03: Wallet created on first payment action (OnchainKit Smart Wallet) -- from Plan 01
- PAY-04: Payments via USDC on Base -- useUSDCTransfer with writeContractAsync
- PAY-05: Platform 5% fee -- split logic in useUSDCTransfer
- PAY-06: 95% reaches agent wallet -- split logic in useUSDCTransfer
...
- QUEU-04: Tips increase rotation weight -- POST /api/tip after successful transfer
</success_criteria>
